...
uint8_t const I2C_DELAY_USEC = 10;
...
class SoftI2cMaster { 
 public:
  ...
  bool writeDelay(uint8_t b, uint8_t delay);
  
  /** Read data from the fuel gauge
   * \param[in] the thing you want to read
   *     0x1C for state of charge - returns a percentage between 0 and 100
   *     0x04 for voltage - returns mV between 0 and 6000
   * \return the data that was read 
   */
  uint16_t fuelGaugeRead(uint8_t target);
